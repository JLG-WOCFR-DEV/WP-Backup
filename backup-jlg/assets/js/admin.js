...
jQuery(function($) {
    ...
    const navLinks = document.querySelectorAll('.nav-tab[data-section], .bjlg-sidebar__nav-link');
    ...
    const setActiveSection = function(sectionKey, updateHistory, fromTab) {
        ...
        navLinks.forEach(function(link) {
            ...
            link.classList.toggle('nav-tab-active', matches && link.classList.contains('nav-tab'));
            if (matches) {
    ...
    if (appEl) {
        const nativeTabs = document.querySelector('#bjlg-main-content .nav-tab-wrapper');
        const datasetSections = parseJSONSafe(appEl.getAttribute('data-bjlg-sections'), []);
        if (Array.isArray(datasetSections)) {
            datasetSections.forEach(function(section) {
                if (!section || typeof section.key === 'undefined') {
                    return;
                }
                const key = String(section.key);
        ...
        const datasetModules = parseJSONSafe(appEl.getAttribute('data-bjlg-modules'), null);
        if (datasetModules && typeof datasetModules === 'object') {
            Object.keys(datasetModules).forEach(function(key) {
                sectionModulesMap[key] = datasetModules[key];
            });
