...
class BJLG_Azure_Blob implements BJLG_Destination_Interface {
    ...
    private const ERROR_USAGE_API_FAILURE = 'AZURE_USAGE_API_ERROR';
    private const ERROR_USAGE_EMPTY = 'AZURE_USAGE_API_EMPTY';
    ...
    public function get_storage_usage() {
        ...
        if (($snapshot['status'] ?? '') !== 'ok') {
            ...
            $error_message = isset($snapshot['error']) && $snapshot['error'] !== ''
                ? $snapshot['error']
                : __('Azure Blob : impossible de récupérer les statistiques de conteneur.', 'backup-jlg');
        ...
        if (class_exists(BJLG_Debug::class)) {
            BJLG_Debug::log(sprintf(
                'Métriques Azure Blob : used=%s quota=%s free=%s (latence=%sms)',
    ...
    public function get_remote_quota_snapshot() {
        ...
        if (!$this->is_connected()) {
            $snapshot['error'] = __('Azure Blob Storage n\'est pas configuré.', 'backup-jlg');
        ...
        try {
            $raw_snapshot = $this->fetch_container_usage_snapshot($settings);
        } catch (Exception $exception) {
            $snapshot['status'] = 'error';
            $snapshot['error_code'] = self::ERROR_USAGE_API_FAILURE;
            $snapshot['error'] = sprintf(
                'Azure Blob : impossible de récupérer les statistiques de conteneur — %s',
        ...
        if (!is_array($raw_snapshot) || (
            ...
            $snapshot['error'] = __('Azure Blob : la réponse de l’API `comp=stats` est vide.', 'backup-jlg');
